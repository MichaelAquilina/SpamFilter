...
Unfortunately, choosing a ideal \(\alpha \) and \(\beta\) is a somewhat tedious process of trial and error. \todo{Uwe you could start writing about feature selection here}
...
\begin{align*}
	tfidf_{ij} = \frac{f_{ij}}{maxf} \times log(\frac{N}{n_i})
\end{align*}

Where $N$ represents the number of documents, $n_i$ represents the number of times the term i appears in a document and $maxf$ represents the highest frequency in the document corpus.